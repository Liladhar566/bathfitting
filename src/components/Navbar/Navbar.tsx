    ...
    <header className={`${styles.header} ${scrolled ? styles.scrolled : ''}`}>
      <nav className={styles.nav} aria-label="Main navigation">
        <div className={styles.inner}>
          {/* Logo */}
          <Link to="/" className={styles.logo} aria-label={clientConfig.businessName}>
            {clientConfig.logoImage ? (
              <img
                src={clientConfig.logoImage}
                alt={clientConfig.businessName}
                className={styles.logoImg}
              />
            ) : (
              <span className={styles.logoText}>{clientConfig.logoText}</span>
            )}
          </Link>
        ...
        >
          <ul className={styles.mobileLinks} role="list">
            {navLinks.map((link, i) => (
              <li
                key={link.to}
                style={{ '--delay': `${i * 0.06 + 0.05}s` } as React.CSSProperties}
              >
                <Link
                  to={link.to}
                  className={`${styles.mobileLinkItem} ${location.pathname === link.to ? styles.mobileActive : ''}`}
                >
                  {link.label}
                </Link>
              </li>
